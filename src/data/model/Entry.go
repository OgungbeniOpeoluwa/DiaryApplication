...
func NewEntry(title string, body string, diaryId string) *Entry {
	times := time.Date(time.Now().Year(), time.Now().Month(), time.Now().Day(), time.Now().Hour(),
		time.Now().Minute(), time.Now().Second(), time.Now().Nanosecond(), time.Local)
	date := times.Format("02-Jan-2006")
	return &Entry{title: title, body: body, dateCreated: date, diaryId: diaryId}
